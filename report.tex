...
When Contacts API receives the response of API call, Firefox OS uses same ID to retrieve the DOMRequest Object which send get-contact request from list. Caller can get the request result only if the reference of DOMRequest still come from same domain.   % receive response
