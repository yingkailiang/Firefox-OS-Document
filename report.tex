...
When Contacts API receives the response of API call, Firefox OS uses same ID to retrieve the DOMRequest Object which sent get-contact request from list. The previous dummy DOMRequest Object can be assigned contact information only if the reference of DOMRequest still comes from same domain.   % receive response
